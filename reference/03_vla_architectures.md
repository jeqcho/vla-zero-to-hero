...
- **Parallel decoding**: emit all `k × action_dim` floats in one forward pass with bidirectional attention.
- **L1 regression loss** instead of cross-entropy over bins.
- **Action chunking**: k=8 for single-arm tabletop; **k=25 for bimanual ALOHA**. Use the right one for your robot or you will train against the wrong chunk size.
- 25–50× inference speedup; +20% LIBERO success.

The recommended starting point for OpenVLA fine-tuning. (Caveat: see VLA-0 below for the "do we even need a custom action head" counterpoint.)
...
- **~3.3B params total** (3B PaliGemma backbone + 300M flow-matching action expert).
...
Open humanoid foundation model. **~2.2B params total** (1.34B VLM + 0.86B DiT action expert). Dual-system architecture:
- **System 2**: VLM at ~5–10 Hz (Eagle-2 in N1; **Eagle-2.5** in N1.5+). Scene + language understanding.
...
## Helix (Figure AI, 2025-02) — closed; awareness only

[Announcement](https://www.figure.ai/news/helix). Closed weights. Dual-system: **S2** (7B VLM, 7–9 Hz, semantic intent latent) + **S1** (80M policy, 200 Hz, upper-body continuous control including individual fingers). First VLA reported to output high-rate continuous control across all upper-body DoF on a real humanoid. [Helix 02](https://www.figure.ai/news/helix-02) (2026-02) extends to a "general-purpose humanoid system."

## Gemini Robotics / -ER / 1.5 (Google DeepMind, 2025+) — closed; awareness only

[Paper](https://arxiv.org/abs/2503.20020) · [Blog](https://deepmind.google/models/gemini-robotics/) · [1.5 blog](https://deepmind.google/blog/gemini-robotics-15-brings-ai-agents-into-the-physical-world/) · [On-Device blog](https://deepmind.google/blog/gemini-robotics-on-device-brings-ai-to-local-robotic-devices/)

Variants: **Gemini Robotics** (Gemini 2.0 + action), **-ER** (embodied-reasoning; spatial + point + trajectory prediction), **1.5** (chain-of-thought before acting), **On-Device** (local inference, 50–100 demos to adapt). Closed weights — the load-bearing idea to take away is "think then act" for long-horizon work.

## SmolVLA (Hugging Face, 2025-06)

[Blog](https://huggingface.co/blog/smolvla) · [Doc](https://huggingface.co/docs/lerobot/smolvla) · [Base](https://huggingface.co/lerobot/smolvla_base) · [paper](https://arxiv.org/abs/2506.01844)

**450M total** = ~350M SmolVLM-2 backbone (with **pixel-shuffle to 64 visual tokens/frame** + layer skipping for speed) + ~100M flow-matching action expert. Default chunk size **50**, **10 ODE steps** at inference. Pretrained **only on open community LeRobot datasets** (everything tagged `lerobot` on the Hub).
...
- **RDT-1B** (2024-10, [arXiv](https://arxiv.org/abs/2410.07864)) — ~1.2B diffusion transformer for bimanual. Diffusion happens in the trunk, not a separate head.
...
- **VLA-0** (2025-10, [arXiv](https://arxiv.org/pdf/2510.13054)) — minimal-modification VLA; uses a stock VLM with very simple action handling. Reportedly competitive with OpenVLA-OFT, π0.5-KI, and SmolVLA on LIBERO with zero architecture changes — worth tracking if the "fancy action head" framing turns out to be over-engineered.
